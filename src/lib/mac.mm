...
static bool getBounds(CGWindowID windowID, ow_window_bounds *outputBounds) {
  ...
  if (x && y && width && height) {
    outputBounds->x = [x intValue];
    outputBounds->y = [y intValue];
    outputBounds->width = static_cast<uint32_t>([width intValue]);
    outputBounds->height = static_cast<uint32_t>([height intValue]);
    return true;
  }
...
static void hookProcTargetWindow(AXObserverRef observer, AXUIElementRef element,
  ...
  CFStringRef moveResizeNotifications[] = {kAXMovedNotification, kAXResizedNotification};
  for (size_t i = 0; i < sizeof(moveResizeNotifications) / sizeof(moveResizeNotifications[0]); i++) {
    if ([notificationType isEqualToString:(__bridge NSString *)moveResizeNotifications[i]]) {
